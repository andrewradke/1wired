...
my ($device, $command, $test, $statecheck, $ConfigFile, $timeout, $verbose, $MinuteMin, $help);
...
my $LowStateFile = '/etc/1wired/low_state';
my $UseRRDs = 0;
...
#	or the check is for < x and the range is >= 5 min and the 5 min min is < x
          if ((m/^<(.*)/) && (checkrange($data{$test},$_))) {
            ...
            print "$range minute(s) minimum $test: $FiveMinute;  $normal\n";
            exit 0;
          } else {
            ($start, $step, $data) = check_rrds($name) if (! defined($data));
            $min = restrict_num_decimal_digits(MinInRange($start, $step, $data, $range),1);
